class Player {
        ...
        document.addEventListener('keydown', (e) => {
            ...
            switch(e.keyCode) {
                ...
                case key_s: // sキー
                    ...
                    e.preventDefault(); return false;
                case arrow_up: // ↑キー
                    this.keyStatus.center_change=true;
                    
                    e.preventDefault(); return false;
                case arrow_down: // ↓キー
                    this.keyStatus.movable_change = true;
                    e.preventDefault(); return false;
            }
        });
        document.addEventListener('keyup', (e) => {
            ...
            switch(e.keyCode) {
                ...
                case key_s: // sキー
                    ...
                    e.preventDefault(); return false;
                 case arrow_up: // ↑キー
                    this.keyStatus.center_change = false;
                    e.preventDefault(); return false;
                case arrow_down: // sキー
                    this.keyStatus.movable_change = false;
                    e.preventDefault(); return false;
            }
        });
        // タッチ操作追加
    ...
    static playing(frame) {
        ...
        } else if(this.keyStatus.center_change||this.keyStatus.movable_change) {
            //ぷよ変更
            console.log('今のぷよの色。センター:'+this.centerPuyo);
            console.log('今のぷよの色。ムーブぷよ:'+this.movablePuyo);
      ...
      // 操作用に作成したぷよ画像を消す
        ...
        if(this.keyStatus.movable_change){
            //今のムーブぷよのいろ
            let mp = this.movablePuyo
            //ムーブぷよに1を足す
            mp +=1;
            //6以上になった場合のために剰余を入れる
            mp = (mp % 5)+1 ;
            console.log('変更後の色:'+mp)
            this.movablePuyo=mp;
        }else if(this.keyStatus.center_change){
         //今のセンターぷよのいろ
            let cp = this.centerPuyo
            //センターぷよに1を足す
            cp +=1;
            //6以上になった場合のために剰余を入れる
            cp = (cp % 5)+1 ;
            console.log('変更後の色:'+cp)
            this.centerPuyo=cp;
        }
        // 新しいぷよ画像を作成する
        this.centerPuyoElement = PuyoImage.getPuyo(this.centerPuyo);
